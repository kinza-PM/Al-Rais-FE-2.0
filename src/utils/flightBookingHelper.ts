...
export const validatePassengersForFlightProvisionalBooking = (fareBookingRules: any, flightBookingPayload: FlightInitialBooking) => {
    ...
    for (let i = 0; i < passengers.length; i++) {
        ...
        if (pRules.isDateOfBirthMandatory) {
            ...
            if (!bd) return { valid: false, error: prefixFor(i, "Birth date is required.") };
            const bdDate = new Date(`${bd}T00:00:00`);
            bdDate.setHours(0, 0, 0, 0);
            if (bdDate > today) return { valid: false, error: prefixFor(i, "Birth date cannot be in the future.") };
        }

        if (pRules.isExpiryDateMandatory) {
            const exp = id.expiryDate ?? null;
            if (!exp) return { valid: false, error: prefixFor(i, "Expiry date is required.") };
            const expDate = new Date(`${exp}T00:00:00`);
            expDate.setHours(0, 0, 0, 0);
            if (expDate < today) return { valid: false, error: prefixFor(i, "Expiry date must be booking date or a future date.") };
        }

        const ruleChecks: Array<[boolean, any, string]> = [
            [pRules.isIdTypeMandatory, id.idType, "ID type is required."],
            [pRules.isDocumentNumberMandatory, id.idDocumentNumber, "Document number is required."],
            [pRules.isIssuingCountryCodeMandatory, id.issuingCountryCode, "Issuing country is required."],
            [pRules.isDateOfIssueMandatory, id.dateOfIssue, "Date of issue is required."],
            [pRules.isResidenceCountryCodeMandatory, id.residenceCountryCode, "Residence country is required."],
            [fareBookingRules?.isLeadEmailAddressMandatory, email, "Email is required."],
            [
                fareBookingRules?.isLeadPhoneNumberMandatory,
                (phone && (phone.areaCode || phone.phoneNumber)) ? `${phone.areaCode || ""}${phone.phoneNumber || ""}` : undefined,
                "Phone (country code and number) is required.",
            ],
            [pRules.isPANMandatory, pi.PAN, "PAN is required."],
            [pRules.isAdditionalIdTypeMandatory, addId.type, "Additional ID type is required."],
            [pRules.isAdditionalDocumentNumberMandatory, addId.number, "Additional document number is required."],
            ...
            [pRules.isOtherAncillaryMandatory, p.otherAncillary, "Other ancillaries are required."],
        ];

        for (const [flag, value, msg] of ruleChecks) {
            if (flag && isEmpty(value)) return { valid: false, error: prefixFor(i, msg) };
        }
    }
