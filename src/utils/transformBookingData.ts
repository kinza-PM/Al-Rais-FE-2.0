...
function calculateCountdown(createdAt: string): { hours: string; mins: string; secs: string } {
...
export function transformBookingItem(apiItem: any): any {
  ...
  const bookingRef = status === "Confirmed" 
  ...
  const countdown = status === "Pending" && apiItem.createdAt
    ? calculateCountdown(apiItem.createdAt)
    : null;
  
  // Transform each journey
  const transformedJourneys = journeys.map((journey: any, journeyIndex: number) => {
    ...
    const marketingAirline = firstSegment.marketingAirline || firstSegment.operatingAirline || "";
    const flightNumber = firstSegment.flightNumber || "";
    
    // Get cabin class
    const cabinClass = firstSegment.cabinClass || firstSegment.priceClassName || "Economy";
    
    // Get duration for the journey
    const journeyDuration = journey?.flight?.flightInfo?.duration || "";
    
    // Check if direct (no stops)
    const isDirect = flightSegments.length === 1 && journey?.flight?.stopQuantity === 0;
...
export function transformBookingToFlightBookingFormat(booking: any): any {
  ...
  const passengersForRequest = (apiItem.request?.passengers || []).map((p: any) => ({
  ...
  return {
    offerId: apiItem.offerId,
    searchKey: apiItem.flightDetails?.[0]?.id || apiItem.offerId?.split("-")[0] || "",
    flightDetail,
