  ...
  if (!hasMultipleSegments) {
    ...
    return (
      <div className="flex flex-col items-center mb-2">
        {duration && (
          <span className="text-[10px] text-[#3D495C] mb-1">Duration {duration}</span>
        )}
...
function FlightJourneyCard({ journey, isLast: _isLast }: { journey: any; isLast: boolean }) {
  return (
    <>
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-8 max-[768px]:grid-cols-[.5fr_auto_.5fr]">
        ...
        <div className="text-center">
          <div className={`text-[15px] font-medium text-[#0A0C0F] ${journey.segments.length > 1 ? "relative bottom-3" : ""}`}>
            {journey.from.code} <span className="mx-2">→</span> {journey.to.code}
          </div>
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <div className="text-[12px] text-[#3D495C]">
                ...
                {journeys[0].airline?.cabin ? ` - ${journeys[0].airline.cabin}` : null}
              </div>
            </div>
          </div>
        )}
