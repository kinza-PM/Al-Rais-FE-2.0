...
const onUpdatesToggle = (checked: boolean) => {
  if (import.meta.env.DEV) {
    // Placeholder until notification preferences are wired for hotel search
    console.debug(`Hotel search updates preference: ${checked}`);
  }
};

/**
 * Figma: full-width CTA between hotel results — only for signed-out users.
 */
const HotelSearchSignInUpdatesBanner: React.FC = () => {
  const { isAuthenticated } = useAuth();
  const handleChange = useCallback((checked: boolean) => {
    onUpdatesToggle(checked);
  }, []);

  if (isAuthenticated) return null;

  return (
    <div className="reminderCard" role="region" aria-label="Sign in for updates">
      ...
      <div>
        <Switch defaultChecked onChange={handleChange} aria-label="Email updates for this search" />
      </div>
    </div>
  );
};

export default HotelSearchSignInUpdatesBanner;
