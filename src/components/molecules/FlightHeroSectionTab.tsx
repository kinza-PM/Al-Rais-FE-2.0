...
const FlightHeroSection: React.FC = () => {
    ...
    const handlePassengers = useCallback((next: Record<string, number>, order: string[]) => {
        setPaxCounts(next);
        setPaxOrder(order);
    }, []);

    const handleDepartDate = useCallback((d: Date | null) => setDepartDate(d), []);
    const handleArrivalDate = useCallback((d: Date | null) => setArrivalDate(d), []);
    ...
    const flightTypesFailed = !!errorMap?.flightTypes || (!loading && (!flightTypes || flightTypes.length === 0));
