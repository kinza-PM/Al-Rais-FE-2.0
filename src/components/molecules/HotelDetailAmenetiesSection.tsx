...
const HotelDetailAmenetiesSection = ({
  ...
  const displayPopularFacilities = useMemo(() => {
    ...
    hotelDetail.hotelFacilities.forEach((facility: any) => {
      ...
      if (unwantedKeywords.some((k) => lowerName.includes(k))) return;
    ...
    <div className="mt-10">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-8">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-4 text-[#3D495C] text-sm">
            {displayPopularFacilities.length > 0 ? (
              displayPopularFacilities.map((facility, index) => (
                <div className="flex items-center gap-2" key={index}>
                  {facility.icon && (
                    <img src={facility.icon} alt="icon" className="w-5 h-5" />
                  )}
