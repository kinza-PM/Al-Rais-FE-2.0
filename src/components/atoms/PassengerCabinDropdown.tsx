...
import type { PassengerSchema, CabinClassOption } from "../../features/flights/types";
...
const PassengerCabinDropdown: React.FC<Props> = ({
    ...
    const [paxLocal, setPaxLocal] = useState<Pax>({ adults: 1, kids: 0, infants: 0, seniors: undefined });
    ...
    const selectedCabinLabel =
        cabinClasses.find(c => c.id === selectedCabinClassId)?.label || "Please select";
    ...
    return (
        <div className={`relative ${widthClass}`} ref={ref}>
            <label className="block text-[12px] text-[#3D495C] mb-1">Passengers &amp; cabin class</label>
            ...
            <button
                ...
                aria-expanded={open}
            >
                <span className="tabular-nums">{pad2(total)} Passengers, {selectedCabinLabel}</span>
            ...
            {open && (
                <div className="absolute z-30 mt-2 w-[320px] rounded-2xl bg-white border border-[#E7EEF7]
                    ...
                    <div className="mb-3">
                        ...
                        <PassengerCounterDropdown
                            value={value}
                            onChange={(p) => { onChangePax ? onChangePax(p) : setPaxLocal(p); }}
                            maxTotal={maxTotal}
                            schema={schema}
                        />
                    </div>
