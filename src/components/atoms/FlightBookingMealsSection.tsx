...
export default function FlightBookingMealsSection({
    ...
    const QtyStepper = ({ cat, item }: { cat: "meals" | "drinks"; item: string }) => {
        const qty = (picks[activePax][cat] as Record<string, number>)[item] || 0;
        if (!qty) return null;
        return (
            <div className="ml-3 inline-flex items-center gap-2">
                <button
                    type="button"
                    onClick={() => changeQty(cat, item, -1)}
                ...
                <span className="w-8 text-center text-[14px] font-medium text-[#0A0C0F]">{String(qty).padStart(2, "0")}</span>
                <button
                    type="button"
                    onClick={() => changeQty(cat, item, 1)}
                    className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#2351A3] text-white hover:brightness-95"
            ...
            <div className="rounded-xl border border-[#E4E4E7] overflow-hidden">
                ...
                >
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <div className="text-[15px] font-medium text-[#0A0C0F]">Meals & Drinks</div>
                            <div className="text-[12px] text-[#3D495C]">
                                Choose from a selection of hot meals and cold drinks served during your flight.
                            </div>
                        </div>
                    </div>
                                    ...
                                    {flightBookingMeals.map(m => {
                                        ...
                                        return (
                                            <li key={m.name} className="flex items-center justify-between py-2">
                                                ...
                                                {checked && <QtyStepper cat="meals" item={m.name} />}
                                            </li>
                                        );
                                    })}
                                </ul>
                                    ...
                                    {flightBookingDrinks.map(d => {
                                        ...
                                        return (
                                            <li key={d.name} className="flex items-center justify-between py-2">
                                                ...
                                                {checked && <QtyStepper cat="drinks" item={d.name} />}
                                            </li>
                                        );
                                    })}
                                </ul>
