...
export default function FlightSummaryCard({
  ...
  const showCancellationFooter =
    isCancellation && (!!fare || !!footerPassengers || !!footerBookingRef);

  const shellClass = isCancellation
    ? "rounded-[16px] border-[1.5px] border-[#C2CAD6] bg-white shadow-sm"
    : "rounded-[16px] border-[1.5px] border-[#C2CAD6] bg-[#efefef] shadow-sm";

  const headerDividerClass = "border-b border-[#E4E4E7]";

  const segmentDividerClass = "border-t border-[#E4E4E7]";
    ...
    <div className={`${shellClass} ${className}`}>
      ...
      {segments.map((seg, i) => (
        <React.Fragment key={i}>
          <div className="px-5 py-3">
            ...
            <div className="mb-4 text-center text-[16px] font-medium text-[#0A0C0F]">
              {seg.route}
            </div>
            ...
            <div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
              <div className="text-left">
                ...
                <div className="text-[12px] text-[#3D495C]">{seg.dep.date}</div>
              </div>
              ...
              <div className="text-right">
                ...
                <div className="text-[12px] text-[#3D495C]">{seg.arr.date}</div>
              </div>
            </div>
          </div>

          {i < segments.length - 1 && <div className={segmentDividerClass} />}
        </React.Fragment>
      ))}
      ...
      {showCancellationFooter && (
        <>
          ...
          <div className="rounded-b-[16px] px-4 py-4">
                ...
                <div className="flex shrink-0 gap-4 sm:gap-5">
                  {footerPassengers ? (
                    <div className="w-[min(112px,40vw)] shrink-0 sm:w-[118px]">
                      <div className="text-[12px] text-[#3D495C]">
                        {footerPassengers.label ?? "Passengers"}
                      </div>
                      <div className="mt-1 text-[15px] font-medium leading-tight text-[#0A0C0F]">
                        {footerPassengers.value}
                      </div>
                    </div>
                  ) : null}
                  {footerBookingRef ? (
                    <div className="w-auto shrink-0 sm:w-auto">
                      <div className="text-[12px] text-[#3D495C]">
                        {footerBookingRef.label ?? "Booking ref. number"}
                      </div>
                      <div className="mt-1 whitespace-nowrap text-[15px] font-medium leading-tight text-[#0A0C0F]">
